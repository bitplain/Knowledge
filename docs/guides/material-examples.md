...
## Галерея изображений (опционально с `glightbox`)

**Назначение:** показывать интерфейсы, схемы и скриншоты без перегрузки страницы.

> Чтобы использовать, установите плагин `mkdocs-glightbox` и добавьте `- glightbox` в блок `plugins` (по умолчанию плагин отключён, чтобы избежать зависимостей).
